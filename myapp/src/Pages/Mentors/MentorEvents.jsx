...
const EventsPage = () => {
  ...
  // Filter events for Mentor role
  const mentorEvents = getFilteredEvents(events);
  const todayEvents = mentorEvents.filter((event) => event.date === today);
  const upcomingEvents = mentorEvents.filter((event) => event.date > today);

  // Generate calendar events data
  const getEventData = (date) => {
    const formattedDate = date.format("YYYY-MM-DD");
    return mentorEvents.filter((event) => event.date === formattedDate);
  };
  ...
  return (
    <Layout style={{ padding: "24px" }}>
      <Content>
        ...
        >
          <h1>Events Management - Mentor</h1>
        </div>

        {/* All Events for Mentor */}
        <div style={{ marginBottom: "34px", fontSize: "25px" }}>
          <h2>My Events (Mentor)</h2>
          <List
            grid={{ gutter: 16, column: 3 }}
            dataSource={mentorEvents}
            loading={loading}
            renderItem={(event) => (
              <List.Item>
                <Card
